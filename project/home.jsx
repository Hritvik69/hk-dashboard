  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, maxHeight: 220, overflowY: 'hidden' }}>
      ...
      {/* Notes - separate panel */}
      <Panel
        label="Notes"
        title={`${openNotes} notes`}
      >
        <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
          <TextInput value={draftText} onChange={setDraftText} placeholder="Write a note..." onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addItem('note'); }}} style={{ flex: 1 }} />
          <Btn variant="accent" onClick={() => addItem('note')}>+ Add Note</Btn>
        </div>
          ...
          {notes.map(note => (
            <div key={note.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 500, marginBottom: note.long ? 4 : 0 }}>{note.text}</div>
                {note.long && <div style={{ fontSize: 12, color: t.textDim }}>{note.long}</div>}
                <div style={{ fontSize: 10, color: t.textMute, marginTop: 6 }}>{note.date}</div>
              </div>
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                <button style={{ fontSize: 9, padding: '4px 8px', background: 'rgba(255,255,255,0.04)', borderRadius: 5, color: t.textDim, fontWeight: 700, letterSpacing: '0.1em' }}>COPY</button>
                <button onClick={() => removeNote(note.id)} style={{ fontSize: 9, padding: '4px 8px', background: hexToRgba(t.danger, 0.1), border: `1px solid ${hexToRgba(t.danger, 0.3)}`, borderRadius: 5, color: t.danger, fontWeight: 700, letterSpacing: '0.1em' }}>DEL</button>
              </div>
            </div>
          ))}
          {notes.length === 0 && <div style={{ fontSize: 13, color: t.textMute, textAlign: 'center', padding: 24 }}>No notes yet</div>}
        </div>
      </Panel>
      ...
      >
        <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
          <TextInput value={draftText} onChange={setDraftText} placeholder="Add a task..." onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addItem('task'); }}} style={{ flex: 1 }} />
          <select
            value={draftPriority}
          ...
          <Btn variant="primary" onClick={() => addItem('task')}>+ Add Task</Btn>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {tasks.map(task => (
            <div key={task.id} style={{
              ...
              <span style={{ flex: 1, fontSize: 14, textDecoration: task.done ? 'line-through' : 'none', color: task.done ? t.textMute : t.text }}>{task.text}</span>
          ...
          {tasks.length === 0 && <div style={{ fontSize: 13, color: t.textMute, textAlign: 'center', padding: 24 }}>No tasks yet</div>}
        </div>
      </Panel>
    </div>
  );
